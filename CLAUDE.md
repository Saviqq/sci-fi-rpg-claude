...
This project is in the **design/pre-production phase**. The repository currently contains only design documents:
- `overview.md` — full game concept and design document
- `demo.md` — vertical slice (MVP) specification

No engine, build system, or source code has been set up yet.

## Game Concept

**Genre:** 2D top-down pixel-art RPG (combat + stealth + dialogue)  
...
- Feral Sense (proximity warning cue)
...
**Engine:** Unity 6.3 LTS, 2D top-down pixel-art, real-time combat.

Core systems — keep them decoupled:

1. **PlayerController** — movement, dodge/stamina, melee attack
2. **FeralSenseSystem** — passive proximity detection; triggers warning cue (visual/audio) when Ferals enter range
3. **EnemyAI** — Feral behavior (erratic movement, burst attack pattern); designed to be variant-extensible (stalker/brute/signaler)
4. **DialogueSystem** — choice tree with binary trust outcome affecting world state (shortcut access)
5. **InteractSystem** — E-key proximity interactions (logs, doors, NPCs)
6. **UIManager** — health bar, Feral Sense cue, dialogue box